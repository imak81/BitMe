...
var route = {
	processRoute : function(req, resp, config){
		...
		for(i = 0, rLen = d.length;  i < rLen; i++){
				...
				return function(result, event, error){
					...
					if(event && event === 'exception'){
						...
						packet = {
							...
							action: so.action,
							method: so.method,
							result: result 			// This shall get the first parameter of the function
													// callback() being called in the API function.
													// The format of the return shold be like this: 
													// {success: true, ... } so at the font end Sencha
													// shall use it (success property) to determine to 
													// trigger the failure/sucess process
						};
			...
			}.apply(null, [so]);	// The function callback() shall always be called with scope = null
									// and additional array param [so], and so that makes variables so
									// besome available inside the return function of callback
			...
			try {
				// Call the API method from the scope = {}
				// Then the API shall be executed and it MUST call the function so.data.callback 
